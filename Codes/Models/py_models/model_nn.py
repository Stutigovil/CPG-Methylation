...
class NNModel:
    def __init__(self, input_dim, hidden_dims=[256, 128, 64], dropout_rate=0.3,
                 learning_rate=1e-3, batch_size=32, epochs=100, random_state=42, verbose=True):
    ...
    def _print_device_info(self):
        ...
        if self.device.type == 'cuda':
            ...
            print(f"  Available: {total_memory - allocated_memory:.2f} GB")
        else:
            print(f"⚠ No GPU detected - using CPU")
            print(f"  To use GPU, ensure:")
            print(f"    1. CUDA-compatible GPU is installed")
            print(f"    2. PyTorch with CUDA support is installed")
            print(f"    3. CUDA drivers are properly configured")
    ...
    def clear_gpu_cache(self):
        ...
        if self.device.type == 'cuda':
            ...
            print("Not using GPU - nothing to clear")
    ...
    def train(self, X, y, X_val=None, y_val=None, early_stopping_patience=10, verbose=True):
        ...
        for epoch in range(self.epochs):
            ...
            if verbose and (epoch + 1) % 10 == 0:
                log_msg = f"Epoch {epoch+1}/{self.epochs} - Loss: {avg_train_loss:.4f} - Val AUC: {val_auc:.4f}"
                if self.device.type == 'cuda':
                    gpu_mem = torch.cuda.memory_allocated(0) / 1024**3
                    log_msg += f" - GPU Mem: {gpu_mem:.2f}GB"
                print(log_msg)
